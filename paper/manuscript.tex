...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figure1_architecture.png}
...
\begin{table}[h]
\centering
\caption{Semantic Definition and Clinical Grounding of Shared Concepts ($K_s=8$)}
\label{tab:concept_semantics}
\begin{tabular}{lll}
\toprule
\textbf{Concept} & \textbf{Target Physiological Entity} & \textbf{Key Biomarkers \& Radiographic Signs} \\
\midrule
$c_1$: Resp-Fail & Acute Hypoxemic Respiratory Failure & $\text{PaO}_2/\text{FiO}_2 < 200$, SpO$_2$, Diffuse Infiltrates \\
...
\subsubsection{Human-in-the-Loop Test-Time Concept Intervention}
A fundamental operational vulnerability in conventional deep clinical decision support is the inability of bedside practitioners to correct an erroneous intermediate inference without retraining the entire network. In XM-CBM, because the classifier head is strictly linear ($\hat{y} = \sigma(w^\top c_{\text{shared}} + b_0)$), clinicians can perform test-time counterfactual interventions. If an attending physician identifies a spurious concept activation (for example, motion blur or hardware artifact erroneously elevating $c_6 = \text{Pulm-Edema}$), the physician can directly override the concept activation:
\begin{equation}
    c_{\text{shared}, k} \leftarrow c_k^\ast, \quad c_k^\ast \in [0, 1]
\end{equation}
The updated mortality risk is instantly recomputed in $\mathcal{O}(1)$ time without gradient backpropagation:
\begin{equation}
    \hat{y}_{\text{intervened}} = \sigma\left(w^\top c_{\text{shared}} + b_0 + w_k(c_k^\ast - c_{\text{shared}, k})\right)
\end{equation}
This test-time intervention capability empowers clinicians to perform interactive ``what-if'' sensitivity simulations (e.g., assessing the marginal reduction in mortality risk if aggressive fluid resuscitation resolves metabolic hypoperfusion $c_2 \to 0$), transforming the system from passive surveillance into an active therapeutic planning tool.

\subsubsection{Missing Modality Resilience and Graceful Degradation}
...
\midrule
\textbf{XM-CBM (Ours)} & \textbf{Tab+Img} & $0.854 \pm 0.012$ & $0.593 \pm 0.024$ & $0.103 \pm 0.007$ & $\mathbf{0.029 \pm 0.006}^\ast$ & $\mathbf{0.142 \pm 0.011}^\ast$ & $\mathbf{0.823 \pm 0.009}^\ast$ & $\mathbf{0.034 \pm 0.005}^\ast$ \\
\bottomrule
\multicolumn{9}{l}{\footnotesize $^\ast$Statistically significant improvement over Late Fusion + IG via two-sided Wilcoxon signed-rank test ($p < 0.001$ across 5 folds).}
\end{tabular}
\end{table*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figure2_main_benchmark.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figure3_roc_pr_curves.png}
...
\begin{table*}[t]
\centering
\caption{Clinical Classification Performance and Diagnostic Error Metrics on Hold-Out Test Split ($N=2,570$)}
\label{tab:clinical_metrics}
\begin{tabular}{llccccccc}
\toprule
\textbf{Model Architecture} & \textbf{Modality} & \textbf{Accuracy} $\uparrow$ & \textbf{Balanced Acc} $\uparrow$ & \textbf{Sensitivity (TPR)} $\uparrow$ & \textbf{Specificity (TNR)} $\uparrow$ & \textbf{Precision (PPV)} $\uparrow$ & \textbf{NPV} $\uparrow$ & \textbf{F1-Score} $\uparrow$ \\
\midrule
XGBoost + TreeSHAP & Tabular & $0.841$ & $0.776$ & $0.684$ & $0.869$ & $0.480$ & $0.940$ & $0.564$ \\
LightGBM + TreeSHAP & Tabular & $0.846$ & $0.783$ & $0.694$ & $0.872$ & $0.490$ & $0.942$ & $0.575$ \\
Tabular MLP + KernelSHAP & Tabular & $0.832$ & $0.758$ & $0.653$ & $0.864$ & $0.459$ & $0.934$ & $0.539$ \\
DenseNet-121 + Grad-CAM & Imaging & $0.811$ & $0.731$ & $0.617$ & $0.845$ & $0.413$ & $0.926$ & $0.494$ \\
ViT-B/16 + Attention & Imaging & $0.820$ & $0.744$ & $0.635$ & $0.853$ & $0.433$ & $0.929$ & $0.515$ \\
Late Fusion + IG & Tab+Img & $\mathbf{0.875}$ & $\mathbf{0.828}$ & $\mathbf{0.762}$ & $\mathbf{0.895}$ & $\mathbf{0.562}$ & $\mathbf{0.955}$ & $\mathbf{0.647}$ \\
\midrule
\textbf{XM-CBM (Ours)} & \textbf{Tab+Img} & $0.869$ & $0.821$ & $0.751$ & $0.890$ & $0.547$ & $0.953$ & $0.633$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figure6_confusion_matrices.png}
\caption{Multi-model confusion matrix breakdown on the test cohort ($N=2,570$, 386 deceased, 2,184 survivors): (a) XGBoost tabular baseline, (b) DenseNet-121 imaging baseline, (c) Late Fusion multimodal black-box, and (d) XM-CBM proposed architecture. XM-CBM captures $75.1\%$ of high-risk mortality events while sustaining $89.0\%$ specificity and delivering an inherently faithful explanation for every alert.}
\label{fig:confusion_matrices}
\end{figure*}

\subsection{Diagnostic Error Distribution and Confusion Matrices}
To assess practical clinical decision thresholds beyond threshold-free summary statistics (AUROC/AUPRC), we evaluate the discrete classification error profiles on the hold-out test cohort ($N=2,570$; $386$ mortality events, $2,184$ ICU survivors) in Table~\ref{tab:clinical_metrics} and Fig.~\ref{fig:confusion_matrices}. 
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figure4_faithfulness_curves.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figure5_qualitative_cases.png}
