...
\author{Mtechbro,~et~al.
\thanks{Mtechbro is with the Department of Computer Science and Engineering and Division of Biomedical Informatics. (e-mail: corresponding.author@institution.edu).}
\thanks{Manuscript received September 20, 2026; revised October 15, 2026. This work was supported in part by the National Institutes of Health (NIH) and open health data initiatives.}}

\markboth{IEEE Transactions on Medical Imaging / IEEE Journal of Biomedical and Health Informatics,~Vol.~XX, No.~X,~2026}%
{Mtechbro \MakeLowercase{\textit{et al.}}: Faithful by Design: A Cross-Modal Concept Bottleneck Framework}
...
\textbf{[Figure 1: Architectural Schematic of XM-CBM]}\\[1ex]
\small
Tabular EHR ($x_{\text{tab}} \in \mathbb{R}^{22}$) $\xrightarrow{\text{MLP}}$ Tabular Concepts ($c_{\text{tab}} \in [0,1]^{K_t}$) $\searrow$\\[0.5ex]
\hspace{8cm} $\xrightarrow{\text{Bidirectional Cross-Attention}}$ Fused Concepts ($c_{\text{fused}}$) $\to$ Bottleneck ($c_{\text{shared}} \in [0,1]^{K_s}$) $\xrightarrow{\text{Linear}}$ Logit $\hat{y}$\\[0.5ex]
Chest Radiograph ($x_{\text{img}}$) $\xrightarrow{\text{DenseNet-121}}$ Image Concepts ($c_{\text{img}} \in [0,1]^{K_v}$) $\nearrow$
}}
\caption{System architecture of the Cross-Modal Concept Bottleneck Model (XM-CBM). Heterogeneous patient signals are projected onto interpretable clinical concept vectors, contextually aligned through multi-head cross-attention, compressed into a shared concept bottleneck, and classified via a transparent linear predictor.}
...
As presented in Table~\ref{tab:benchmark}, multimodal models significantly outperform unimodal baselines. Fusing tabular EHR with chest radiographs boosts AUROC by $+0.048$ over the best tabular model (LightGBM: $0.819 \to 0.867$) and $+0.093$ over the best vision model (ViT: $0.774 \to 0.867$). 

Comparing the unconstrained black-box Late Fusion model against XM-CBM demonstrates a nominal accuracy trade-off: XM-CBM incurs a minor drop of $0.013$ in AUROC ($0.854$ vs. $0.867$) and $0.019$ in AUPRC ($0.593$ vs. $0.612$). However, XM-CBM achieves the lowest Expected Calibration Error across all tested architectures ($\text{ECE} = 0.029 \pm 0.006$). Constraining predictions through bounded concepts acts as an effective structural regularizer, mitigating the overconfidence typically exhibited by deep unconstrained networks.
